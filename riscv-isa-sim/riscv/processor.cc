...
std::string TRACE_INFO_FILEPATH = "/Users/siemensixone/Documents/Diploma/Files/instructions_output.txt";
...
void processor_t::disasm(insn_t insn)
{
  ...
  if (last_pc != state.pc || last_bits != bits) {
    ...
    s << "core " << std::dec << std::setfill(' ') << std::setw(3) << id
      ...
      //MODIFIED
      //
      std::ofstream my_custom_file;
      my_custom_file.open(TRACE_INFO_FILEPATH, std::ios::app);
      if (my_custom_file.is_open()){
          my_custom_file 
          /*mcycle*/ << (state.mcycle)->read()
          << "|"
          /*pc*/ << std::hex << "0x" << std::setfill('0') << std::setw(max_xlen / 4) << zext(state.pc, max_xlen)
          << "|"
          /*instruction*/ << disassembler->disassemble(insn)
          << std::endl;
      }
      my_custom_file.close();
      //
      //
      //
    debug_output_log(&s);
